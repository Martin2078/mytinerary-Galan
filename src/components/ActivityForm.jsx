...
const ActivityForm = ({ dataItinerary, setDataItinerary, setActivityAdd, activityPhotos, setActivityPhotos }) => {
    ...
    function renderPhotos() {
        ...
        for (let i = 0; i < urlPhotos.length; i++) {
            template.push(<img className='h-4/5 lg:h-full w-[30vw] lg:w-[12vw] xl:w-[7vw] object-cover rounded-xl' src={urlPhotos[i]} alt="" />)
        }
        return template
    }
    ...
    return (
        <div className='w-full lg:w-[35vw] animationMyItinerary relative h-screen bg-white flex flex-col items-center justify-start gap-2 px-4 lg:px-8 rounded-l-2xl'>
            ...
            <div className='w-full h-3/5 lg:h-full py-5 flex flex-col justify-start gap-6 overflow-y-auto lg:overflow-hidden'>
                <div className='w-full md:w-5/6 lg:w-full flex flex-col gap-2 '>
                    <div className='flex items-center justify-between w-full'>
                        <p className='font-semibold text-xl'>Name</p>
                        <p className={`lg:text-sm text-xs text-red-600 opacity-0 ${activityError.name && "opacity-100"}`}>* Obligatory field</p>
                    </div>
                ...
                <div className='w-full  flex flex-col gap-2 '>
                    <div className='flex items-center justify-between w-full md:w-4/5'>
                        ...
                        <p className={`lg:text-sm text-xs text-red-600 opacity-0 ${activityError.photo && "opacity-100"}`}>* Obligatory field</p>
                    </div>
                    {urlPhotos.length > 0
                        ?
                        <div className='w-full h-[20vh] lg:h-[15vh] flex flex-col gap-2'>
                            <div className='w-full h-4/5 flex gap-5 overflow-x-auto'>
                                {renderPhotos()}
                            </div>
                            <div className='w-full lg:w-2/6 h-1/5 bg-[#2dc77f] border flex items-center rounded-xl justify-center relative'>
                        ...
                        :
                        <div className={`bg-gray-100 relative w-10/12 md:w-6/12 lg:w-10/12 xl:w-8/12 h-[15vh] lg:h-[12vh] border ${activityError.photo ? "border-red-600" : "border-black"} flex items-center justify-center rounded-lg`}>
                ...
                <div className='w-full md:w-5/6 lg:w-full flex flex-col gap-2 items-end'>
                    <div className='w-full flex items-center justify-between '>
                        <p className='font-semibold text-xl'>Description</p>
                        <p className={`lg:text-sm text-xs text-center text-red-600 opacity-0 ${activityError.description && "opacity-100"}`}>* Obligatory field (min 50 characters)</p>
                    </div>
                ...
                <div className='w-full md:w-5/6 lg:w-full flex flex-col gap-2 '>
                    <div className='flex items-center justify-between'>
                        ...
                        <p className={`lg:text-sm text-xs text-red-600 opacity-0 ${activityError.ubication && "opacity-100"}`}>* Obligatory field</p>
                    </div>
            ...
            <div className='w-full flex items-center justify-center mb-12 gap-2 lg:gap-4'>
                <button className='cursor-pointer w-3/6 py-2 bg-[#b92525] rounded-xl text-base lg:text-xl font-semibold text-white' onClick={() => { setActivity({ name: "", description: "", photo: [], ubication: "" }); setActivityAdd(false) }}>
                    Cancel
                </button>
                <button className='cursor-pointer w-3/5 py-2 bg-[#2dc77f] rounded-xl text-base lg:text-xl font-semibold text-white' onClick={() => addActivity()}>
                    Add activity
                </button>
            </div>
        </div>
